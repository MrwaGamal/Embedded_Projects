...
void LCD_initallize()
{
	...
	DIO_VsetPINDIR(portname,4,1); // those four lines and the DIO write low level 
	DIO_VsetPINDIR(portname,5,1); // will change if i connect the LCD to the most pins of mc port
...
{  
	...
	#elif defined four_bit_mode
	DIO_write_high_level(portname,data>>4);
	DIO_vwrite(controlport,RS,1);
	ENABLE_PULSE();
	DIO_write_high_level(portname,data);
	DIO_vwrite(controlport,RS,1);
	ENABLE_PULSE();
	#endif
...
void LCD_send_cmd(char cmd)
{
	...
	#elif defined four_bit_mode
	DIO_write_high_level(portname,cmd>>4);
	DIO_vwrite(controlport,RS,0);
	ENABLE_PULSE();
	DIO_write_high_level(portname,cmd);
	DIO_vwrite(controlport,RS,0);
	ENABLE_PULSE();
	#endif
